...
export default function Chat(props) {
  const chatArray = chatEvents.map((event, index) => {
    let eventName = participants.filter(item => item.id === chatEvents[index].participantId)[0].name;
    let eventPrefix = '';
    if (chatEvents[index].type !== 'message') {
      eventPrefix = `${eventName} gives a ${chatEvents[index].type}`;
    }
        return (
        <div>
        <h3>{eventName}
